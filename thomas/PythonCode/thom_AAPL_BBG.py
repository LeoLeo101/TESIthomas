...
# ------------------- 1) Prezzi & colonne -------------------
prices_raw = read_prices_txt(PRICES_TXT, start_date="2000-01-01")
...
for ym, g in dftech.groupby("YearMonth", sort=True):
    rets.append({
        "YearMonth": ym,
        "AAPL_ret": month_last_first_ret(g, "AAPL") if "AAPL" in g else np.nan,
        "NOK_ret":  month_last_first_ret(g, "NOK")  if "NOK"  in g else np.nan,
...
# Excess returns
if "AAPL_ret" in df: df["AAPL_ret_adj"] = df["AAPL_ret"] - df["RF"]
if "NOK_ret"  in df: df["NOK_ret_adj"]  = df["NOK_ret"]  - df["RF"]

# ------------------- 4) Regressioni CAPM / FF3F ------------
def safe_series(name): 
    return df[name] if name in df else pd.Series(index=df.index, dtype=float)

capm_AAPL = run_ols(safe_series("AAPL_ret_adj"), df[["Mkt.RF"]])
capm_NOK  = run_ols(safe_series("NOK_ret_adj"),  df[["Mkt.RF"]])

print("\n=== CAPM AAPL (AAPL-RF ~ Mkt.RF) ===")
print(capm_AAPL.summary())
print("\n=== CAPM NOK (NOK-RF ~ Mkt.RF) ===")
print(capm_NOK.summary())

ff3_AAPL = run_ols(safe_series("AAPL_ret_adj"), df[["Mkt.RF", "SMB", "HML"]])
ff3_NOK  = run_ols(safe_series("NOK_ret_adj"),  df[["Mkt.RF", "SMB", "HML"]])

print("\n=== FF3F AAPL (AAPL-RF ~ Mkt.RF + SMB + HML) ===")
print(ff3_AAPL.summary())
print("\n=== FF3F NOK (NOK-RF ~ Mkt.RF + SMB + HML) ===")
print(ff3_NOK.summary())

B_AAPL = float(ff3_AAPL.params.get("Mkt.RF", np.nan))
B_NOK  = float(ff3_NOK.params.get("Mkt.RF",  np.nan))

A_AAPL = float(ff3_AAPL.params["const"]) if "const" in ff3_AAPL.params else np.nan
A_NOK  = float(ff3_NOK.params["const"])  if "const" in ff3_NOK.params  else np.nan

print(f"\nBeta (Mkt.RF) da FF3F -> AAPL: {B_AAPL:.4f}, NOK: {B_NOK:.4f}")
print(f"\nAlpha (Intercept) da FF3F -> AAPL: {A_AAPL:.4f}, NOK: {A_NOK:.4f}")

# ------------------- 5) Cumulate beta-adjusted -------------
df_bad = dftechRet.copy()

if np.isfinite(B_AAPL) and B_AAPL != 0 and "AAPL_ret" in df_bad:
    df_bad["AAPL_ret"] = df_bad["AAPL_ret"] / B_AAPL
else:
    print("Attenzione: beta AAPL non valido; salto l'aggiustamento.")

if np.isfinite(B_NOK) and B_NOK != 0 and "NOK_ret" in df_bad:
    df_bad["NOK_ret"] = df_bad["NOK_ret"] / B_NOK
else:
    print("Attenzione: beta NOK non valido; salto l'aggiustamento.")

if "AAPL_ret" in df_bad: df_bad["AAPL_ret_index"] = base100_from_returns(df_bad["AAPL_ret"])
if "NOK_ret"  in df_bad: df_bad["NOK_ret_index"]  = base100_from_returns(df_bad["NOK_ret"])
if "SPX_ret"  in df_bad: df_bad["SPX_ret_index"]  = base100_from_returns(df_bad["SPX_ret"])

# ------------------- 6) Grafico ----------------------------
plot_cols = [c for c in ["AAPL_ret_index", "NOK_ret_index", "SPX_ret_index"] if c in df_bad.columns]
labels = {"AAPL_ret_index":"AAPL", "NOK_ret_index":"NOK", "SPX_ret_index":"SPX"}
...
plt.title("Rendimenti AAPL vs NOK $\\beta$ adjusted\n"
